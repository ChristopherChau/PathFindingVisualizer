...
export const animateFinalPath = async (
  nodes,
  finalPathNodes,
  intervalDelay,
  resetCallback,
...
export const visualizeAlgorithm = async (params) => {
  ...
  if (currentAlg === "Dijkstra's") {
    ...
    visitedNodesInOrder = params.minHeapDijkstra(
      nodes,
      startNodeObj,
      finishNodeObj
    );
  ...
  await animateAlgorithmFn(
    nodes,
    visitedNodesInOrder,
    params.intervalDelay,
    params.reset,
    params.setNodes,
    params.setAnimationIds
  );

  const finalPath = params.getFinalPath(nodes[finishNode.row][finishNode.col]);
  await animateFinalPathFn(
    nodes,
    finalPath,
    params.intervalDelay,
    params.reset,
